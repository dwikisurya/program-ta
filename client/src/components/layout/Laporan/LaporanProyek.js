...
const LaporanProyek = () => {
    ...
    const rendertableBarangRAB = () => {
        return dataTeamRAB.map(dtr => {
            if (dtr.namaProyek === formData.namaProyek) {
                return (
                    <tr key={dtr.id}>
                        <td> {dtr.idSDB.map(nm => {
                            return <tr><td style={{ height: 100 }}>{nm.namaBarang}<br></br></td></tr>
                        })}
                        </td>
                        <td> {dtr.pcsSDB.map(nm => {
                            return <tr><td style={{ height: 100 }}>{nm + ' pcs'}<br></br></td></tr>
                        })}
                        </td>
                    </tr>
                )
    ...
    const exportPDFRAB = () => {
        ...
        doc.save(`RAB ` + formData.namaProyek + `.pdf`)
    }
    const exportPDFSCHEDULING = () => {
        ...
        doc.save(`Scheduling ` + formData.namaProyek + `.pdf`)
    }
    ...
    const exportPDFPELAPORAN = () => {
        ...
        doc.save(`Perkembangan ` + formData.namaProyek + `.pdf`)
    }
        ...
        <div className="container-fluid">
            ...
            <div className="row" style={{ margin: 10 }}>
                ...
                <div className="row">
                    ...
                    <div className="col-md-4" style={{ visibility: 'hidden', height: 0, width: 0 }}>
                        <h5>Tim RAB</h5>
                        <table className="table table-bordered" id="BarangRAB">
                            <thead>
                                <tr>
                                    <th>Nama Barang</th>
                            ...
                            <tbody>{rendertableBarangRAB()}</tbody>
                        </table>
                        <button type="button" class="btn btn-primary" onClick={() => exportPDFRAB()}>Download</button>
                    </div>
